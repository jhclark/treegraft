 ...
 */
public class ClassFinder<X> {
	private Class<X> searchClass = null;
	 ...
	 */
	public final Vector<Class<? extends X>> findSubclasses(Class<X> clazz) {
			...
			synchronized (results) {
				try {
					...
					if (fqcn.startsWith(".") || fqcn.endsWith(".")) {
						return new Vector<Class<? extends X>>();
					}
					...
					//
					try {
						searchClass = (Class<X>) Class.forName(fqcn);
	...
	private final Vector<Class<? extends X>> findSubclasses(Class<X> superClass,
		...
		Vector<Class<? extends X>> v = new Vector<Class<? extends X>>();

		Vector<Class<? extends X>> w = null; // new Vector<Class<?>> ();
	...
	private final Vector<Class<? extends X>> findSubclasses(URL location, String packageName,
			Class<X> superClass) {
	...
	// v = finder.findSubclasses(Class.forName(args[0]));
